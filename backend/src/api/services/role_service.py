...
def list_roles(limit: int, offset: int, q: Optional[str]):
    rows = role_controller.list_roles(limit, offset, q)
    return [dict(id=r[0], code=r[1], label=r[2], description=r[3], created_at=r[4], updated_at=r[5]) for r in rows]

def get_role(role_id: int):
    r = role_controller.get_role_by_id(role_id)
    if not r : raise ValueError("Role introuvable")
    perms = role_controller.list_permissions_for_role(role_id)
    return {
